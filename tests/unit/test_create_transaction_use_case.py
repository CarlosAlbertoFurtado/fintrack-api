...
class TestCreateTransaction:
    ...
    @pytest.mark.asyncio
    async def test_rejects_category_from_another_user(self, use_case, mock_deps):
        # garante que não dá pra usar categoria de outro user passando o id direto
        dto = CreateTransactionDTO(
            description="Test", amount=100, type="EXPENSE",
            category_id="cat-other-user",
        )
        mock_deps["category_repo"].find_by_id.return_value = Category(
            id="cat-other-user", name="Food", type=CategoryType.EXPENSE,
            user_id="other-user",  # dono diferente
        )
    ...
    @pytest.mark.asyncio
    async def test_creates_without_category_when_ai_returns_none(self, use_case, mock_deps):
        # IA pode retornar None (sem chave config, ou não achou match)
        dto = CreateTransactionDTO(
            description="Random payment", amount=50, type="EXPENSE",
        )
        mock_deps["ai_service"].categorize.return_value = None
        mock_deps["category_repo"].find_by_user.return_value = []
        created_tx = Transaction(
            description="Random payment", amount=50,
            type=TransactionType.EXPENSE, user_id="u1",
        )
        mock_deps["transaction_repo"].create.return_value = created_tx
